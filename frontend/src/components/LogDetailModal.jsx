...
const LogDetailModal = ({ log, onClose, onPrev, onNext }) => {
  ...
  return (
    <div className="fixed inset-0 bg-neutral-950/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-neutral-900/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden max-h-[85vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          ...
          <div className="flex items-center gap-2">
            {onPrev && (
              <button onClick={onPrev} className="p-2 text-gray-400 hover:text-white glass-panel/5 hover:glass-panel/10 rounded-lg transition-colors border border-white/10" title="Previous Log (Left Arrow)">
                <ChevronLeft size={18} />
              </button>
            )}
            {onNext && (
              <button onClick={onNext} className="p-2 text-gray-400 hover:text-white glass-panel/5 hover:glass-panel/10 rounded-lg transition-colors border border-white/10" title="Next Log (Right Arrow)">
                <ChevronRight size={18} />
              </button>
            )}
            <button onClick={onClose} className="p-2 text-gray-400 hover:text-red-400 glass-panel/5 hover:bg-red-500/10 border-red-500/30 text-red-2000/10 rounded-lg transition-colors border border-white/10 ml-2" title="Close (Escape)">
              <X size={18} />
            </button>
          </div>
        </div>
        ...
        <div className="p-5 space-y-5 overflow-y-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="glass-panel/5 border border-white/10 rounded-lg px-4 py-3">
          ...
          <div className="border border-white/10 rounded-lg p-4 glass-panel/5 group">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs text-gray-400">Raw Message</p>
              <CopyButton text={log.rawMessage} />
            </div>
            <pre className="text-xs text-gray-200 whitespace-pre-wrap break-words font-mono leading-relaxed glass-panel/5 border border-white/10 p-3 rounded-lg overflow-x-auto">{log.rawMessage}</pre>
          </div>

          <div className="border border-white/10 rounded-lg p-4 glass-panel/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {entries.map((entry) => (
                <div key={entry.key} className={`space-y-1${entry.fullWidth ? 'md:col-span-2' : ''}`}>
                  ...
                  <div className="rounded-md border border-white/10 glass-panel/5 px-3 py-2 text-sm text-gray-200 font-mono break-words leading-relaxed overflow-x-auto">
                    {entry.value}
                  </div>
                </div>
              ))}
